...
*    What is the maximum value an 8-bit variable can store?The maximum value an 8-bit variable can store is 255. What are some
*    of the benefits and drawbacks of using 8-bit variables in an 8-bit
*    microcontroller? A benefit to using an 8-bit variable in an 8-bit microcontroller is that it will be optimal because it has the same amount of bits as the microcontroller.  
*
...
*    (Hint: try pressing and releasing the button at different rates of speed.) The counter is counting for how long LED3 is on for instead of counting how many times SW2 is pressed and released.
*
...
*    SW2Count variable as the SW2 button is held? The count of the SW2Count variable gets reset back to 0. 
*
...
*    value that the SW2Count variable will reach?255 How does this affect the
*    operation of LED D4 when SW2 is held?Once SW2 is being held and reaches the max count LED D4 will stay on.
*
...
// Program constant definitions
//Max count to turn on the "Hold" LED5
const unsigned char maxCount = 100;
//Max count to turn off the "Hold" LED5
const unsigned char maxCount_off = 200;
//Max count to turn on the "Press" LED4
const unsigned char maxCount_p = 1;
//Max count to turn off the "Press" LED4
const unsigned char maxCount_p2 = 2;
// Program variable definitions
unsigned char SW2CountD5 = 0;
unsigned char SW2CountD4 = 0;
...
int main(void)
{
   ...
   while(1)
   {
       ...
       if(SW2CountD5 >= maxCount)
       {
           LED5 = 1;
       }
       if(LED5 == 1 && SW2CountD5 >= maxCount_off)
       {
           LED5 = 0;
           SW2CountD5 = 0;
       }
//push button press
           if(SW2p && SW2Pressed == false)
            {
       ...
       //clears pressed state if released
       if(SW2np)
       {
           LED3 = 0;
           SW2Pressed = false;
       }
       if(SW2CountD4 == maxCount_p)
       {
           LED4 = 1;
       }
       //Turn off LED4 and reset all counters
           if(SW2CountD4 > maxCount_p)
           {
               LED4 = 0;
               SW2CountD4 = 0;
